...
class OpossumInACanBubba(Npc):
    def __init__(self, x: int, y: int):
        super().__init__(x, y)

        # Integrated textbox content into guy_messages
        self.selected_item_index = 0
        self.opossum_in_can_bubba_messages = {
            "welcome_message": NpcTextBox(
                ["Bubba: Are you sure you want to battle the king of Opossum?"],
                (50, 450, 700, 130), 36, 500),
            "defeated_message": NpcTextBox(
                ["Bubba: How, impossible, nobody has every defeated my double opossum technique."],
                (50, 450, 700, 130), 36, 500),
    ...
    def update_waiting(self, state: "GameState"):
        ...
        if (distance < 40 and state.controller.confirm_button and \
            ...
            if Events.OPOSSUM_IN_A_CAN_BUBBA_DEFEATED.value in state.player.level_five_npc_state:
                self.opossum_in_can_bubba_messages["defeated_message"].reset()

            else:
                self.opossum_in_can_bubba_messages["welcome_message"].reset()

    def update_talking(self, state: "GameState"):
        current_message = (
            self.opossum_in_can_bubba_messages["defeated_message"]
            if Events.OPOSSUM_IN_A_CAN_BUBBA_DEFEATED.value in state.player.level_five_npc_state
            else self.opossum_in_can_bubba_messages["welcome_message"]
        )
        ...
        if (current_message.is_finished() and Events.OPOSSUM_IN_A_CAN_BUBBA_DEFEATED.value
            ...
            if selected_option == "Yes" and state.player.stamina_points > 0 and state.player.money >= 50:
                ...
                state.currentScreen = state.opossumInACanBubbaScreen
                state.opossumInACanBubbaScreen.start(state)
