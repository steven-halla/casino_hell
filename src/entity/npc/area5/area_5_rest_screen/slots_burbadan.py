...
class SlotsBurbadan(Npc):
    def __init__(self, x: int, y: int):
        ...
        self.slots_burbadan = {
            "welcome_message": NpcTextBox(
                ["Burbadan: Are you sure you want to battle the king of slots?"],
                (50, 450, 700, 130), 36, 500),
            "defeated_message": NpcTextBox(
                ["Burbadan: How, impossible, nobody has every defeated my double slots technique."],
                (50, 450, 700, 130), 36, 500),
    ...
    def update_waiting(self, state: "GameState"):
        ...
        if (distance < 40 and state.controller.confirm_button and \
            ...
            if Events.SLOTS_BURBADAN_DEFEATED.value in state.player.level_five_npc_state:
                self.slots_burbadan["defeated_message"].reset()

            else:
                self.slots_burbadan["welcome_message"].reset()

    def update_talking(self, state: "GameState"):
        current_message = (
            self.slots_burbadan["defeated_message"]
            if Events.SLOTS_BURBADAN_DEFEATED.value in state.player.level_five_npc_state
            else self.slots_burbadan["welcome_message"]
        )
        ...
        if (current_message.is_finished() and Events.SLOTS_BURBADAN_DEFEATED.value
