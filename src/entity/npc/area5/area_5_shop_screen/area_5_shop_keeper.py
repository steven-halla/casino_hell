...
class Area5ShopKeeper(Npc):
    def __init__(self, x: int, y: int):
        ...
        self.shop_items = [Equipment.COIN_SAVE_AREA_5.value, Equipment.CRAPS_WRIST_WATCH.value, Equipment.STAT_POTION_AREA_5.value, Equipment.CHEFS_HAT.value, Equipment.MP_BRACELET.value,Equipment.MEDIUM_VEST.value ]
    ...
    def update(self, state: "GameState"):
        ...
        if self.stat_point_increase == True:
            state.area5RestScreen.shop_lock = True
            ...
            if (state.controller.isTPressed or state.controller.isAPressedSwitch) and pygame.time.get_ticks() - self.input_time > 400:
                ...
                if self.stat_point_increase_index == 0 and state.player.body < 3:
                    state.player.body += 1
                    state.player.max_stamina_points += state.player.level_5_body_stamina_increase
                    state.player.stamina_points += state.player.level_5_body_stamina_increase

                    self.stat_point_increase = False
                    state.area5RestScreen.shop_lock = False


                elif self.stat_point_increase_index == 1 and state.player.mind < 3:
                    state.player.mind += 1
                    state.player.max_focus_points += state.player.level_5_mind_focus_increase
                    state.player.focus_points += state.player.level_5_mind_focus_increase
        ...
        if self.state == "waiting":
            ...
            if Equipment.COIN_SAVE_AREA_5.value in state.player.level_five_npc_state:
                self.shop_items[0] = "sold out"

            if Equipment.CRAPS_WRIST_WATCH.value in state.player.level_five_npc_state:
                self.shop_items[1] = "sold out"

            if Equipment.STAT_POTION_AREA_5.value in state.player.level_five_npc_state:
                self.shop_items[2] = "sold out"

            if Equipment.CHEFS_HAT.value in state.player.level_five_npc_state:
                self.shop_items[3] = "sold out"

            if Equipment.MP_BRACELET.value in state.player.level_five_npc_state:
                self.shop_items[4] = "sold out"
            if Equipment.MEDIUM_VEST.value in state.player.level_five_npc_state:
            ...
            if self.textbox.message_index == 0 and self.textbox.is_finished():
                ...
                if state.controller.confirm_button:
                    ...
                    if self.selected_item_index == 0:
                        print("Yes")
                        cost = 1200
                        if state.player.money - cost < 500 or Equipment.COIN_SAVE_AREA_5.value in state.player.level_five_npc_state:
                            self.cant_buy_sound.play()  # Not enough money left or already purchased
                        else:
                            self.buy_sound.play()
                            if state.player.enhanced_luck:
                                state.player.luck -= 1
                            Equipment.COIN_SAVE_AREA_5.add_shop_items_to_player_inventory(state.player, Equipment.COIN_SAVE_AREA_5)
                        ...
                        cost = 1000
                        if state.player.money - cost < 500 or Equipment.CRAPS_WRIST_WATCH.value in state.player.level_five_npc_state:
                            self.cant_buy_sound.play()
                        else:
                            self.buy_sound.play()
                            Equipment.CRAPS_WRIST_WATCH.add_equipment_to_player_level_5(state.player, Equipment.CRAPS_WRIST_WATCH)
                            state.player.money -= cost

                    elif self.selected_item_index == 2:
                        cost = 1000
                        if state.player.money - cost < 500 or Equipment.CHEFS_HAT.value in state.player.level_five_npc_state:
                            self.cant_buy_sound.play()
                        else:
                            self.buy_sound.play()
                            Equipment.CHEFS_HAT.add_equipment_to_player_level_5(state.player, Equipment.HEALTHY_GLOVES)
                            state.player.money -= cost

                    elif self.selected_item_index == 3:
                        cost = 1000
                        if state.player.money - cost < 500 or Events.STAT_POTION_AREA_5.value in state.player.level_five_npc_state:
                            self.cant_buy_sound.play()
                        else:
                            self.buy_sound.play()
                            Equipment.STAT_POTION_AREA_5.add_shop_items_to_player_inventory(state.player, Equipment.STAT_POTION_AREA_5)
                            state.player.money -= cost
                            self.stat_point_increase = True

                    elif self.selected_item_index == 4:
                        cost = 1000
                        if state.player.money - cost < 500 or Equipment.MP_BRACELET.value in state.player.level_five_npc_state:
                            self.cant_buy_sound.play()
                        else:
                            Equipment.MP_BRACELET.add_equipment_to_player_level_5(state.player, Equipment.BOSS_KEY)
                            state.player.money -= cost
                            self.buy_sound.play()
                    elif self.selected_item_index == 5:
                        cost = 1500
                        if state.player.money - cost < 500 or Equipment.MEDIUM_VEST.value in state.player.level_five_npc_state:
                            self.cant_buy_sound.play()
                        else:
                            Equipment.MEDIUM_VEST.add_equipment_to_player_level_5(state.player, Equipment.MEDIUM_VEST)
                            state.player.money -= cost
                            self.buy_sound.play()
    ...
    def draw(self, state):
        ...
        if self.state == "waiting":
            ...
            if self.state == "talking":
                ...
                if self.selected_item_index == 0 and Equipment.COIN_SAVE_AREA_5.value not in state.player.level_five_npc_state:
                    ...
                    state.DISPLAY.blit(self.font.render(f"Saves your game.", True,
                                                        (255, 255, 255)), (70, 510))
                if self.selected_item_index == 0 and Equipment.COIN_SAVE_AREA_5.value in state.player.level_five_npc_state:
                    state.DISPLAY.blit(self.font.render(f"Save Coin is Sold out!", True,
                                                        (255, 255, 255)), (70, 460))



                if self.selected_item_index == 1 and Equipment.CRAPS_WRIST_WATCH.value not in state.player.level_five_npc_state:
                ...
                elif self.selected_item_index == 1 and Equipment.CRAPS_WRIST_WATCH.value in state.player.level_five_npc_state:
                    state.DISPLAY.blit(self.font.render(f"Craps Wrist Watch is sold out!", True,
                                                        (255, 255, 255)), (70, 460))



                if self.selected_item_index == 2 and Equipment.CHEFS_HAT.value not in state.player.level_five_npc_state:
                ...
                elif self.selected_item_index == 2 and Equipment.CHEFS_HAT.value in state.player.level_five_npc_state:
                    state.DISPLAY.blit(self.font.render(f"Chef's Hat is sold out!", True,
                                                        (255, 255, 255)), (70, 460))

                if self.selected_item_index == 3 and Events.STAT_POTION_AREA_5.value not in state.player.level_five_npc_state:
                ...
                elif self.selected_item_index == 3 and Events.STAT_POTION_AREA_5.value in state.player.level_five_npc_state:
                    state.DISPLAY.blit(self.font.render(f"Stat Potion is sold out!", True,
                                                        (255, 255, 255)), (70, 460))

                if self.selected_item_index == 4 and Equipment.MP_BRACELET.value not in state.player.level_five_npc_state:
                ...
                elif self.selected_item_index == 4 and Equipment.MP_BRACELET.value in state.player.level_five_npc_state:
                    state.DISPLAY.blit(self.font.render(f"MP Bracelet is sold out!", True,
                                                        (255, 255, 255)), (70, 460))

                if self.selected_item_index == 5 and Equipment.MEDIUM_VEST.value not in state.player.level_five_npc_state:
                ...
                elif self.selected_item_index == 5 and Equipment.MEDIUM_VEST.value in state.player.level_five_npc_state:
