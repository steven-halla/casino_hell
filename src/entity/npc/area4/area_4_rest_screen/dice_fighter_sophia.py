...
class DiceFighterSophia(Npc):
    def __init__(self, x: int, y: int):
        super().__init__(x, y)

        # Integrated textbox content into guy_messages
        self.selected_item_index = 0
        self.sophia_messages = {
            "welcome_message": NpcTextBox(
                ["Sophia: Are you sure you want to battle the king of Battle DICE?"],
                (50, 450, 700, 130), 36, 500),
            "defeated_message": NpcTextBox(
                ["Sophia: How, impossible, nobody has every defeated my double draw technique."],
                (50, 450, 700, 130), 36, 500),
    ...
    def update_waiting(self, state: "GameState"):
        ...
        if (distance < 40 and state.controller.confirm_button and \
            ...
            if Events.DICE_FIGHTER_SOPHIA_DEFEATED.value in state.player.level_four_npc_state:
                self.sophia_messages["defeated_message"].reset()

            else:
                self.sophia_messages["welcome_message"].reset()

    def update_talking(self, state: "GameState"):
        current_message = (
            self.sophia_messages["defeated_message"]
            if Events.DICE_FIGHTER_SOPHIA_DEFEATED.value in state.player.level_four_npc_state
            else self.sophia_messages["welcome_message"]
        )
        ...
        if (current_message.is_finished() and Events.DICE_FIGHTER_SOPHIA_DEFEATED.value
            ...
            if selected_option == "Yes" and state.player.stamina_points > 0 and state.player.money >= 50:
                ...
                state.currentScreen = state.diceFighterSophiaScreen
                state.diceFighterSophiaScreen.start(state)
