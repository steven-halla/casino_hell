...
class BlackJackJasmine(Npc):
    def __init__(self, x: int, y: int):
        super().__init__(x, y)

        # Integrated textbox content into guy_messages
        self.selected_item_index = 0
        self.coin_flip_jasmine_messages = {
            "welcome_message": NpcTextBox(
                ["Jasmine: Are you sure you want to battle the queen of black jack?"],
                (50, 450, 700, 130), 36, 500),
            "defeated_message": NpcTextBox(
                ["Jasmine: How, impossible, nobody has every defeated my double draw technique."],
                (50, 450, 700, 130), 36, 500),
        }
        self.choices = ["Yes", "No"]
        self.menu_index = 0
    ...
    def update_waiting(self, state: "GameState"):
        ...
        if (distance < 40 and state.controller.confirm_button and \
            ...
            if Events.BLACK_JACK_JASMINE_DEFEATED.value in state.player.level_four_npc_state:
                self.coin_flip_jasmine_messages["defeated_message"].reset()

            else:
                self.coin_flip_jasmine_messages["welcome_message"].reset()

    def update_talking(self, state: "GameState"):
        current_message = (
            self.coin_flip_jasmine_messages["defeated_message"]
            if Events.BLACK_JACK_JASMINE_DEFEATED.value in state.player.level_four_npc_state
            else self.coin_flip_jasmine_messages["welcome_message"]
        )
