...
class CrapsNaba(Npc):
    def __init__(self, x: int, y: int):
        ...
        self.craps_naba_messages = {
            "welcome_message": NpcTextBox(
                ["Naba: Are you sure you want to battle the king of Craps?"],
                (50, 450, 700, 130), 36, 500),
            "defeated_message": NpcTextBox(
                ["Naba: How, impossible, nobody has every defeated my double flip technique."],
                (50, 450, 700, 130), 36, 500),
        }
        self.choices = ["Yes", "No"]
        self.menu_index = 0
    ...
    def update_waiting(self, state: "GameState"):
        ...
        if (distance < 40 and state.controller.confirm_button and \
            ...
            if Events.CRAPS_NABA_DEFEATED.value in state.player.level_four_npc_state:
                self.craps_naba_messages["defeated_message"].reset()

            else:
                self.craps_naba_messages["welcome_message"].reset()

    def update_talking(self, state: "GameState"):
        current_message = (
            self.craps_naba_messages["defeated_message"]
            if Events.CRAPS_NABA_DEFEATED.value in state.player.level_four_npc_state
            else self.craps_naba_messages["welcome_message"]
        )
        ...
        if (current_message.is_finished() and Events.CRAPS_NABA_DEFEATED.value
            ...
            if selected_option == "Yes" and state.player.stamina_points > 0 and state.player.money >= 50:
                ...
                state.currentScreen = state.crapsNabaScreen
                state.crapsNabaScreen.start(state)
