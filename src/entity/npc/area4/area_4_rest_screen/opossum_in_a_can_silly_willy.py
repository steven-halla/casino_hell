...
class OpossumInACanSillyWilly(Npc):
    def __init__(self, x: int, y: int):
        super().__init__(x, y)

        # Integrated textbox content into guy_messages
        self.selected_item_index = 0
        self.opossum_in_a_can_silly_willy_messages = {
            "welcome_message": NpcTextBox(
                ["Silly Willy: Are you sure you want to battle the king of oppossum in a can?"],
                (50, 450, 700, 130), 36, 500),
            "defeated_message": NpcTextBox(
                ["Silly Willy: How, impossible, nobody has every defeated my opossums."],
                (50, 450, 700, 130), 36, 500),
        }
        self.choices = ["Yes", "No"]
        self.menu_index = 0
    ...
    def update_waiting(self, state: "GameState"):
        ...
        if (distance < 40 and state.controller.confirm_button and \
            ...
            if Events.OPOSSUM_IN_A_CAN_SILLY_WILLY_DEFEATED.value in state.player.level_four_npc_state:
                self.opossum_in_a_can_silly_willy_messages["defeated_message"].reset()
            else:
                self.opossum_in_a_can_silly_willy_messages["welcome_message"].reset()

    def update_talking(self, state: "GameState"):
        current_message = (
            self.opossum_in_a_can_silly_willy_messages["defeated_message"]
            if Events.OPOSSUM_IN_A_CAN_SILLY_WILLY_DEFEATED.value in state.player.level_four_npc_state
            else self.opossum_in_a_can_silly_willy_messages["welcome_message"]
        )
        ...
        if (current_message.is_finished() and Events.OPOSSUM_IN_A_CAN_SILLY_WILLY_DEFEATED.value
            ...
            if selected_option == "Yes" and state.player.stamina_points > 0 and state.player.money >= 50:
                ...
                state.currentScreen = state.opossumInACanSillyWillyScreen
                state.opossumInACanSillyWillyScreen.start(state)
    ...
    def draw(self, state):
        ...
        if self.state == "talking":
            ...
            if (current_message.is_finished() and Events.OPOSSUM_IN_A_CAN_SILLY_WILLY_DEFEATED.value
                    not in state.player.level_four_npc_state and current_message.message_at_end()):
                bet_box_width = 150
                bet_box_height = 100
                border_width = 5
