...
class HighLowCody(Npc):
    def __init__(self, x: int, y: int):
        super().__init__(x, y)

        # Integrated textbox content into guy_messages
        self.selected_item_index = 0
        self.high_low_cody_messages = {
            "welcome_message": NpcTextBox(
                ["Cody: Are you sure you want to battle the Queen of High Low??"],
                (50, 450, 700, 130), 36, 500),
            "defeated_message": NpcTextBox(
                ["Cody: How, impossible, nobody has every defeated my Cards."],
                (50, 450, 700, 130), 36, 500),
        }
        self.choices = ["Yes", "No"]
        self.menu_index = 0
    ...
    def update_waiting(self, state: "GameState"):
        ...
        if (distance < 40 and state.controller.confirm_button and \
            ...
            if Events.HIGH_LOW_CODY_DEFEATED.value in state.player.level_four_npc_state:
                self.high_low_cody_messages["defeated_message"].reset()

            else:
                self.high_low_cody_messages["welcome_message"].reset()

    def update_talking(self, state: "GameState"):
        current_message = (
            self.high_low_cody_messages["defeated_message"]
            if Events.HIGH_LOW_CODY_DEFEATED.value in state.player.level_four_npc_state
            else self.high_low_cody_messages["welcome_message"]
        )
        ...
        if (current_message.is_finished() and Events.HIGH_LOW_CODY_DEFEATED.value
            ...
            if selected_option == "Yes" and state.player.stamina_points > 0 and state.player.money >= 50:
                state.currentScreen = state.highLowCodyScreen
                state.highLowCodyScreen.start(state)
