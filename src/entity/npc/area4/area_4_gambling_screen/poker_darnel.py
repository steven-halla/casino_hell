...
class PokerDarnel(Npc):
    def __init__(self, x: int, y: int):
        super().__init__(x, y)

        self.slots_juragan_messages = {
            "welcome_message": NpcTextBox(
                ["Darbel: Are you sure you want to battle the king of oink oink poker?"],
                (50, 450, 700, 130), 36, 500),
            "defeated_message": NpcTextBox(
                ["darnel: How, impossible, nobody has every defeated my oinky oink."],
                (50, 450, 700, 130), 36, 500),
    ...
    def update_waiting(self, state: "GameState"):
        ...
        if (distance < 40 and state.controller.confirm_button and \
            ...
            if Events.POKER_DARNEL_DEFEATED.value in state.player.level_four_npc_state:
                self.slots_juragan_messages["defeated_message"].reset()

            else:
                self.slots_juragan_messages["welcome_message"].reset()

    def update_talking(self, state: "GameState"):
        current_message = (
            self.slots_juragan_messages["defeated_message"]
            if Events.POKER_DARNEL_DEFEATED.value in state.player.level_four_npc_state
            else self.slots_juragan_messages["welcome_message"]
        )
        ...
        if (current_message.is_finished() and Events.POKER_DARNEL_DEFEATED.value
            ...
            if selected_option == "Yes" and state.player.stamina_points > 0 and state.player.money >= 50:
                ...
                state.currentScreen = state.pokerDarnelScreen
                state.pokerDarnelScreen.start(state)
