...
class WheelOfTortureBoss(Npc):
    def __init__(self, x: int, y: int):
        super().__init__(x, y)

        # Integrated textbox content into guy_messages
        self.selected_item_index = 0
        self.wheel_of_torture_vanessa_black_messages = {
            "welcome_message": NpcTextBox(
                ["Boss: Are you sure you want to play wheel of tortre?"],
                (50, 450, 700, 130), 36, 500),
            "defeated_message": NpcTextBox(
                ["Boss: How, impossible, nobody has every defeated my wheel."],
                (50, 450, 700, 130), 36, 500),
        }
        self.choices = ["Yes", "No"]
        self.menu_index = 0
    ...
    def update_waiting(self, state: "GameState"):
        ...
        if (distance < 40 and state.controller.confirm_button and \
            ...
            if Events.WHEEL_OF_TORTURE_BOSS_DEFEATED.value in state.player.level_four_npc_state:
                self.wheel_of_torture_vanessa_black_messages["defeated_message"].reset()

            else:
                self.wheel_of_torture_vanessa_black_messages["welcome_message"].reset()

    def update_talking(self, state: "GameState"):
        current_message = (
            self.wheel_of_torture_vanessa_black_messages["defeated_message"]
            if Events.WHEEL_OF_TORTURE_BOSS_DEFEATED.value in state.player.level_four_npc_state
            else self.wheel_of_torture_vanessa_black_messages["welcome_message"]
        )
        ...
        if (current_message.is_finished() and Events.WHEEL_OF_TORTURE_BOSS_DEFEATED.value
            ...
            if selected_option == "Yes" and state.player.stamina_points > 0 and state.player.money >= 50:
                ...
                state.currentScreen = state.wheelOfTortureBossScreen
                state.wheelOfTortureBossScreen.start(state)
