    ...
    def __init__(self, x: int, y: int):
        ...
        self.coin_flip_wanton_messages = {
            "welcome_message": NpcTextBox(
                ["Ted: Are you sure you want to battle the king of COIN FLIP?"],
                (50, 450, 700, 130), 36, 500),
            "defeated_message": NpcTextBox(
                ["Ted: How, impossible, nobody has every defeated my double flip technique."],
                (50, 450, 700, 130), 36, 500),
    ...
    def update_waiting(self, state: "GameState"):
        ...
        if (distance < 40 and state.controller.confirm_button and \
            ...
            if Events.COIN_FLIP_TED_DEFEATED.value in state.player.level_one_npc_state:
                self.coin_flip_wanton_messages["defeated_message"].reset()

            else:
                self.coin_flip_wanton_messages["welcome_message"].reset()

    def update_talking(self, state: "GameState"):
        current_message = (
            self.coin_flip_wanton_messages["defeated_message"]
            if Events.COIN_FLIP_TED_DEFEATED.value in state.player.level_one_npc_state
