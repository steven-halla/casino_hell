...
class BlackJackThomas(Npc):
    def __init__(self, x: int, y: int):
        ...
        self.coin_flip_thomas_messages = {
            "welcome_message": NpcTextBox(
                ["Thomas: Are you sure you want to battle the king of BlackJack FLIP?"],
                (50, 450, 700, 130), 36, 500),
            "defeated_message": NpcTextBox(
                ["Thomas: How, impossible, nobody has every defeated my double BlackJack technique."],
                (50, 450, 700, 130), 36, 500),
    ...
    def update_waiting(self, state: "GameState"):
        ...
        if (distance < 40 and state.controller.confirm_button and \
            ...
            if Events.BLACK_JACK_THOMAS_DEFEATED.value in state.player.level_one_npc_state:
                self.coin_flip_thomas_messages["defeated_message"].reset()

            else:
                self.coin_flip_thomas_messages["welcome_message"].reset()

    def update_talking(self, state: "GameState"):
        current_message = (
            self.coin_flip_thomas_messages["defeated_message"]
            if Events.BLACK_JACK_THOMAS_DEFEATED.value in state.player.level_one_npc_state
            else self.coin_flip_thomas_messages["welcome_message"]
        )
        ...
        if (current_message.is_finished() and Events.BLACK_JACK_THOMAS_DEFEATED.value
