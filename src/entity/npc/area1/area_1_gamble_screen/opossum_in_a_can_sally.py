...
class OpossumInACanSally(Npc):
    def __init__(self, x: int, y: int):
        super().__init__(x, y)

        # Integrated textbox content into guy_messages
        self.selected_item_index = 0
        self.opossum_in_a_can_sallymesages = {
            "welcome_message": NpcTextBox(
                ["Sally: Are you sure you want to battle the king of Opossum In a can?"],
                (50, 450, 700, 130), 36, 500),
            "defeated_message": NpcTextBox(
                ["Sally: How, impossible, nobody has every defeated my Opossum flip technique."],
                (50, 450, 700, 130), 36, 500),
    ...
    def update_waiting(self, state: "GameState"):
        ...
        if (distance < 40 and state.controller.confirm_button and \
            ...
            if Events.OPOSSUM_IN_A_CAN_SALLY_DEFEATED.value in state.player.level_one_npc_state:
                self.opossum_in_a_can_sallymesages["defeated_message"].reset()

            else:
                self.opossum_in_a_can_sallymesages["welcome_message"].reset()

    def update_talking(self, state: "GameState"):
        current_message = (
            self.opossum_in_a_can_sallymesages["defeated_message"]
            if Events.OPOSSUM_IN_A_CAN_SALLY_DEFEATED.value in state.player.level_one_npc_state
            else self.opossum_in_a_can_sallymesages["welcome_message"]
        )
        ...
        if (current_message.is_finished() and Events.OPOSSUM_IN_A_CAN_SALLY_DEFEATED.value
                not in state.player.level_one_npc_state
