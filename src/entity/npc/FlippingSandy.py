...
class FlippingSandy(Npc):
    def __init__(self, x: int, y: int):
        super().__init__(x, y)
        self.flipping_sandy_messages = {
            "welcome_message": NpcTextBox(
                ["Press A to select options and go through T messages", "Welcome to Coin flip I'll make you flip!"],
    ...
    def update_waiting(self, state: "GameState"):
        ...
        if distance < 40 and state.controller.isTPressed and \
            ...
            if state.coinFlipSandyScreen.coinFlipSandyDefeated:
                self.flipping_sandy_messages["defeated_message"].reset()
            else:
                self.flipping_sandy_messages["welcome_message"].reset()

    def update_talking(self, state: "GameState"):
        current_message = self.flipping_sandy_messages["defeated_message"] if state.coinFlipSandyScreen.coinFlipSandyDefeated else self.flipping_sandy_messages["welcome_message"]
