...
class BlackJackAlbert(Npc):
    def __init__(self, x: int, y: int):
        super().__init__(x, y)

        # Integrated textbox content into guy_messages
        self.selected_item_index = 0
        self.black_jack_albert_messages = {
            "welcome_message": NpcTextBox(
                ["ALbert: Are you sure you want to battle the king of BlackJack FLIP?"],
                (50, 450, 700, 130), 36, 500),
            "defeated_message": NpcTextBox(
                ["Albert: How, impossible, nobody has every defeated my double BlackJack technique."],
                (50, 450, 700, 130), 36, 500),
    ...
    def update_waiting(self, state: "GameState"):
        ...
        if (distance < 40 and state.controller.confirm_button and \
            ...
            if Events.BLACK_JACK_ALBERT_DEFEATED.value in state.player.level_three_npc_state:
                self.black_jack_albert_messages["defeated_message"].reset()

            else:
                self.black_jack_albert_messages["welcome_message"].reset()

    def update_talking(self, state: "GameState"):
        current_message = (
            self.black_jack_albert_messages["defeated_message"]
            if Events.BLACK_JACK_ALBERT_DEFEATED.value in state.player.level_three_npc_state
            else self.black_jack_albert_messages["welcome_message"]
        )
