    ...
    def update_waiting(self, state: "GameState"):
        ...
        if (distance < 40 and (state.controller.isTPressed or state.controller.isAPressedSwitch) and \
            ...
            if Events.DICE_FIGHTER_SIR_SIEGFRIED_DEFEATED.value in state.player.level_three_npc_state:
                self.sir_siegfried_messages["defeated_message"].reset()

            else:
                self.sir_siegfried_messages["welcome_message"].reset()

    def update_talking(self, state: "GameState"):
        current_message = (
            self.sir_siegfried_messages["defeated_message"]
            if Events.DICE_FIGHTER_SIR_SIEGFRIED_DEFEATED.value in state.player.level_three_npc_state
            else self.sir_siegfried_messages["welcome_message"]
        )
