...
class OpossumInACanBillyBob(Npc):
    def __init__(self, x: int, y: int):
        super().__init__(x, y)

        # Integrated textbox content into guy_messages
        self.selected_item_index = 0
        self.opossum_in_a_can_billy_bob_messages = {
            "welcome_message": NpcTextBox(
                ["Billy Bob: Are you sure you want to battle the king of oppossum in a can?"],
                (50, 450, 700, 130), 36, 500),
            "defeated_message": NpcTextBox(
                ["Billy Bob: How, impossible, nobody has every defeated my opossums."],
                (50, 450, 700, 130), 36, 500),
    ...
    def update_waiting(self, state: "GameState"):
        ...
        if (distance < 40 and (state.controller.isTPressed or state.controller.isAPressedSwitch) and \
            ...
            if Events.OPOSSUM_IN_A_CAN_BILLY_BOB_DEFEATED.value in state.player.level_three_npc_state:
                self.opossum_in_a_can_billy_bob_messages["defeated_message"].reset()

            else:
                self.opossum_in_a_can_billy_bob_messages["welcome_message"].reset()

    def update_talking(self, state: "GameState"):
        current_message = (
            self.opossum_in_a_can_billy_bob_messages["defeated_message"]
            if Events.OPOSSUM_IN_A_CAN_BILLY_BOB_DEFEATED.value in state.player.level_three_npc_state
            else self.opossum_in_a_can_billy_bob_messages["welcome_message"]
        )
        ...
        if (current_message.is_finished() and Events.OPOSSUM_IN_A_CAN_BILLY_BOB_DEFEATED.value
            ...
            if selected_option == "Yes" and state.player.stamina_points > 0 and state.player.money >= 50:
                ...
                state.currentScreen = state.opossumInACanBillyBobScreen
                state.opossumInACanBillyBobScreen.start(state)
