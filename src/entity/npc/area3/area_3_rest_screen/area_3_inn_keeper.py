    ...
    def update_talking(self, state: "GameState"):
        ...
        if current_message.is_finished() and state.controller.isTPressed:
            ...
            if selected_option == "Yes":
                if Events.QUEST_1_COIN.value in state.player.level_three_npc_state and Events.QUEST_1_BADGE.value not in state.player.level_three_npc_state:
                    state.player.level_three_npc_state.remove(Events.QUEST_1_COIN.value)
                    state.player.quest_items.remove(Events.QUEST_1_COIN.value)

                if Events.QUEST_1_BADGE.value in state.player.level_three_npc_state and Events.QUEST_1_COIN.value not in state.player.level_three_npc_state:
                    state.player.level_three_npc_state.remove(Events.QUEST_1_BADGE.value)
