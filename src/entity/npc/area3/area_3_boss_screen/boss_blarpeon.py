...
class BossBlarpeon(Npc):
    def __init__(self, x: int, y: int):
        ...
        self.craps_happy_messages = {
            "welcome_message": NpcTextBox(
                ["Blarpeon: Are you sure you want to battle the ultimate boss of high/low ?"],
                (50, 450, 700, 130), 36, 500),
            "defeated_message": NpcTextBox(
                ["BlarPeon: How, impossible, nobody has every defeated my boss high/low technique."],
                (50, 450, 700, 130), 36, 500),
    ...
    def update_waiting(self, state: "GameState"):
        ...
        if (distance < 40 and state.controller.confirm_button and \
            ...
            if Events.BOSS_BLARPEON_DEFEATED.value in state.player.level_three_npc_state:
    ...
    def update_talking(self, state: "GameState"):
        ...
        if (current_message.is_finished() and Events.BOSS_BLARPEON_DEFEATED.value
            ...
            if selected_option == "Yes" and state.player.stamina_points > 0 and state.player.money >= 50:
                ...
                state.currentScreen = state.highLowBossScreen
                state.highLowBossScreen.start(state)
