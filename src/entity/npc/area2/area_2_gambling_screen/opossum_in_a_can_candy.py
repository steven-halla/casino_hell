...
class OpossumInACanCandy(Npc):
    def __init__(self, x: int, y: int):
        ...
        self.black_jack_thomas_messages = {
            "welcome_message": NpcTextBox(
                ["Candy: You sure you wanna do battle with a opossum wrangler?"],
                (50, 450, 700, 130), 36, 500),
            "defeated_message": NpcTextBox(
                ["Next time I'll be sure to give my opossums extra rabies."],
                (50, 450, 700, 130), 36, 500),
    ...
    def update_waiting(self, state: "GameState"):
        ...
        if distance < 40 and (state.controller.isTPressed or state.controller.isAPressedSwitch) and \
            ...
            if Events.OPOSSUM_IN_A_CAN_CANDY_DEFEATED.value in state.player.level_two_npc_state:
    ...
    def update_talking(self, state: "GameState"):
        ...
        if current_message.is_finished() and current_message.message_at_end() and (state.controller.isTPressed or state.controller.isAPressedSwitch) and Events.OPOSSUM_IN_A_CAN_CANDY_DEFEATED.value not in state.player.level_two_npc_state and state.player.hasRabies == False:
    ...
    def draw(self, state):
        ...
        if self.state == "talking":
            ...
            if current_message.is_finished() and Events.OPOSSUM_IN_A_CAN_CANDY_DEFEATED.value not in state.player.level_two_npc_state and state.player.hasRabies == False and current_message.message_at_end():
                bet_box_width = 150
                bet_box_height = 100
                border_width = 5
