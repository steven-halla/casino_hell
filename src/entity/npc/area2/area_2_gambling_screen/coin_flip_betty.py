...
class CoinFlipBetty(Npc):
    def __init__(self, x: int, y: int):
        ...
        self.character_sprite_image = pygame.image.load(
            "./assets/images/SNES - Harvest Moon - Mayors Wife.png").convert_alpha()
    ...
    def update_waiting(self, state: "GameState"):
        ...
        if distance < 40 and (state.controller.isTPressed or state.controller.isAPressedSwitch) and \
            ...
            if Events.COIN_FLIP_BETTY_DEFEATED.value in state.player.level_two_npc_state:
    ...
    def update_talking(self, state: "GameState"):
        ...
        if current_message.is_finished() and current_message.message_at_end() and (state.controller.isTPressed or state.controller.isAPressedSwitch) and Events.COIN_FLIP_BETTY_DEFEATED.value not in state.player.level_two_npc_state:
    ...
    def draw(self, state):
        ...
        if self.state == "talking":
            ...
            if current_message.is_finished() and Events.COIN_FLIP_BETTY_DEFEATED.value not in state.player.level_two_npc_state and state.player.hasRabies == False and current_message.message_at_end():
                bet_box_width = 150
                bet_box_height = 100
                border_width = 5
