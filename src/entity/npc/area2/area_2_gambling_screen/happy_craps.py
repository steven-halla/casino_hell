...
class CrapsHappy(Npc):
    def __init__(self, x: int, y: int):
        ...
        self.black_jack_thomas_messages = {
            "welcome_message": NpcTextBox(
                ["Happy Crappy: My name is Happy but everyone calls me Happy Crappy, wanna play some  craps?"],
                (50, 450, 700, 130), 36, 500),
            "defeated_message": NpcTextBox(
                ["I guess now everyone is going to be calling me depressed crappy."],
                (50, 450, 700, 130), 36, 500),
    ...
    def update_talking(self, state: "GameState"):
        ...
        if current_message.is_finished() and current_message.message_at_end() and (state.controller.isTPressed or state.controller.isAPressedSwitch) and Events.CRAPS_HAPPY_DEFEATED.value not in state.player.level_two_npc_state:
            ...
            if selected_option == "Yes":
                state.currentScreen = state.crapsHappyScreen
                state.crapsHappyScreen.start(state)
