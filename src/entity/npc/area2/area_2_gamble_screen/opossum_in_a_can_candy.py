...
class OpossumInACanCandy(Npc):
    def __init__(self, x: int, y: int):
        ...
        self.opossum_in_a_can_candy_mesages = {
            "welcome_message": NpcTextBox(
                ["candy: Are you sure you want to battle the king of Opossum In a can?"],
                (50, 450, 700, 130), 36, 500),
            "defeated_message": NpcTextBox(
                ["Candy: How, impossible, nobody has every defeated my Opossum flip technique."],
                (50, 450, 700, 130), 36, 500),
    ...
    def update_waiting(self, state: "GameState"):
        ...
        if (distance < 40 and state.controller.confirm_button and \
            ...
            if Events.OPOSSUM_IN_A_CAN_CANDY_DEFEATED.value in state.player.level_two_npc_state:
                self.opossum_in_a_can_candy_mesages["defeated_message"].reset()

            else:
                self.opossum_in_a_can_candy_mesages["welcome_message"].reset()

    def update_talking(self, state: "GameState"):
        current_message = (
            self.opossum_in_a_can_candy_mesages["defeated_message"]
            if Events.OPOSSUM_IN_A_CAN_CANDY_DEFEATED.value in state.player.level_two_npc_state
            else self.opossum_in_a_can_candy_mesages["welcome_message"]
        )
        ...
        if (current_message.is_finished() and Events.OPOSSUM_IN_A_CAN_CANDY_DEFEATED.value
