...
class Area2FocusBoost(TreasureChest):
    def __init__(self, x: float, y: float):
        super().__init__(x, y, "Water Bottle")
        self.hidden_item = "an Extra 10 Focus "
        self.isOpened = False
        ...
        self.text_box_messages = {
            "default_message": NpcTextBox(
                [f"You have received {self.hidden_item} Points!"],
                (50, 450, 50, 45), 30, 500
            )
        }
    ...
    def give_item(self, state: "GameState"):
        state.player.focus_points += self.focus_boost
        state.player.max_focus_points += self.focus_boost
        Treasure.add_treasure_to_player(state.player, Treasure.FOCUS_BOOST)
        self.isOpened = True
