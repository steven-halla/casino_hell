...
class CoinFlipSandyScreen(Screen):
    ...
    def flipCoin(self, state: "GameState"):
        ...
        #     self.result = "tails"
        if self.coinFlipSandyMoney > 500:
            self.result = "tails"
        elif self.coinFlipSandyMoney <= 200:
            coin_flip_result = random.randint(1, 20)

            # Check the result
            if coin_flip_result > 6:
                print("Heads")
                self.result = "tails"
            else:
                self.result = "heads"
            ...
            self.result = self.players_last_choice
        ...
        if self.game_state == "results_screen":
            ...
            if not self.has_run_money_logic:
                ...
                if "coin flip glasses" in state.player.items and self.player_choice == self.result:
                    ...
                    self.coinFlipSandyMoney -= self.bet + 20
                    if self.coinFlipSandyMoney == -10:
                        self.coinFlipSandyMoney = 0
                        state.player.money -= 10
                    elif self.coinFlipSandyMoney == -20:
                        self.coinFlipSandyMoney = 0
