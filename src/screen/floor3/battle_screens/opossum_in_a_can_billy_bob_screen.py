...
class OpossumInACanBillyBobScreen(GambleScreen):
    def __init__(self, screenName: str = "Opossum in a can Billy Bob") -> None:
        ...
        self.billyBobOpossumMoney: int = 444
        self.peek_cost: int = 25
        self.buff_peek: bool = False
        self.debuff_double_pick: int = 5
        self.opossum_shoes_chance: int = 0
        self.billy_bob_magic_points: int = 1
    ...
    def opossum_round_reset(self, state):
        ...
        self.opossum_shoes_chance += 3
        inflict_opossum_shoes_randomizer = 0

        match self.billy_bob_magic_points:
            case 2:
                inflict_opossum_shoes_randomizer = random.randint(1, 70) + self.opossum_shoes_chance
            case 1:
    ...
    def update(self, state):
        if self.billyBobOpossumMoney <= self.billy_bob_bankrupt:
        ...
        if self.game_state == self.WELCOME_SCREEN:
            ...
            if self.player_score > self.billyBobOpossumMoney   :
                self.player_score = self.billyBobOpossumMoney
            ...
            if controller.isTPressed or controller.isAPressedSwitch:
                ...
                self.billyBobOpossumMoney -= self.player_score
                if self.billyBobOpossumMoney < 0:
                    self.billyBobOpossumMoney = 0
                self.game_state = self.WELCOME_SCREEN
    ...
    def update_welcome_screen_logic(self, controller, state):
        if controller.isTPressed or controller.isAPressedSwitch:
            ...
            if self.welcome_screen_index == self.pick_index:
                self.billyBobOpossumMoney += self.bet
                self.game_state = self.PICK_TALLY_MENU_SCREEN
    ...
    def draw_box_info(self, state: 'GameState'):
        ...
        state.DISPLAY.blit(self.font.render(f"{self.MONEY_HEADER} {self.billyBobOpossumMoney}", True, WHITE), (player_enemy_box_info_x_position, enemy_money_y_position))
